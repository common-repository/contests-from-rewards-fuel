...
class Contests_From_Rewards_Fuel_Admin extends Class_rf
{
	...
	public function enqueue_styles() {
		wp_enqueue_style( 'contests-from-rewards-fuel', CONTESTS_FROM_REWARDS_FUEL_BASE_URL. 'admin/css/contests-from-rewards-fuel-admin.css', array(),CONTESTS_FROM_REWARDS_FUEL_VERSION, 'all' );
		wp_enqueue_style( 'contests-from-rewards-fuel-dependencies', CONTESTS_FROM_REWARDS_FUEL_BASE_URL. 'admin/css/dependencies.css', array(),CONTESTS_FROM_REWARDS_FUEL_VERSION, 'all' );
	}
	public function enqueue_scripts() {
		wp_enqueue_script( 'contests-from-rewards-fuel-dependencies-popper', 'https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js', array( ),CONTESTS_FROM_REWARDS_FUEL_VERSION, false );
		wp_enqueue_script( 'contests-from-rewards-fuel-dependencies', CONTESTS_FROM_REWARDS_FUEL_BASE_URL . 'admin/js/dependencies.js', array( ),CONTESTS_FROM_REWARDS_FUEL_VERSION, false );
		wp_enqueue_script( 'contests-from-rewards-fuel', CONTESTS_FROM_REWARDS_FUEL_BASE_URL . 'admin/js/contests-from-rewards-fuel-admin.js',array(), CONTESTS_FROM_REWARDS_FUEL_VERSION, true );
	...
	public static function ajax_handler(){
		...
		try {
			if (!defined('DOING_AJAX') || !wp_verify_nonce($_REQUEST['nonce'], 'rf_ajax_nonce')) {
				return;
			}
			...
			if (current_user_can($capability)) {
				...
				if (isset($_REQUEST["update_rewards_fuel_api_key"]) && current_user_can('administrator')) {
					$api_key = sanitize_text_field($_REQUEST["update_rewards_fuel_api_key"]);
                    $api_key= esc_html($api_key);
					update_option("rewards_fuel_api_key", $api_key);
					wp_die();
				}
	...
	public static function rewards_fuel_dashboard_page()
	{
		try {
			...
			wp_enqueue_script( 'contests-from-rewards-fuel-dependencies-bs-table', CONTESTS_FROM_REWARDS_FUEL_BASE_URL . 'admin/js/bootstrap-table.min.js', array( ),CONTESTS_FROM_REWARDS_FUEL_VERSION, false );
			$rf = new Contests_From_Rewards_Fuel();
